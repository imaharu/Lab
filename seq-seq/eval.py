...
def result(encoder, decoder, source_doc):
    ...
    for i in range(0, max_dsn):
        ...
        for word in line:
            ...
            list_ew_hx.append(ew_hx)
    dw_hx, dw_cx = ew_hx, ew_cx
    list_ew_hx = torch.stack(list_ew_hx, 0)
    ...
    while(1):
        ...
        dot = (dw_hx * list_ew_hx).sum(-1, keepdim=True)
        a_t = F.softmax( dot, 0 )
        d = (a_t * list_ew_hx).sum(0)
        concat  = torch.cat((d, dw_hx), 1)
        ht_new = F.tanh(decoder.w_decoder.attention_linear(concat))

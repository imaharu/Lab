...
def train(encoder, decoder, source_doc, target_doc):
    ...
    for i in range(0, max_dsn):
        ...
        for words in lines:
            ...
            ew_hx , ew_cx = encoder(words, ew_hx, ew_cx)
    ...
    for i in range(0, max_dtn):
        ...
        for word_f, word_t in zip(lines_f_last, lines_t_last):
            dw_hx , dw_cx = decoder(word_f, dw_hx, dw_cx)
            loss += F.cross_entropy(decoder.linear(dw_hx), word_t , ignore_index=0)
    return loss
...
if __name__ == '__main__':
    ...
    for epoch in range(10):
        ...
        for i in range(0, train_doc_num, batch_size):
            ...
            target_docs = [ [ [ target_vocab["<bos>"]]  + s + [ target_vocab["<eos>"] ] for s in t_d ] for t_d in target_docs]
